...
async function saveHiddenPrompts(entries) {
  await atomicWrite(HIDDEN_PROMPTS_FILE, `${JSON.stringify(entries, null, 2)}\n`);
}
...
async function computePrompts(platform, agentName, dirOverride) {
  ...
  if (platform === 'hermes') {
    ...
    try {
      ...
      groups = [];
      if (warnings.length < 50) warnings.push(`hermes: failed to read ${dir}: ${error.message}`);
    }
  } else {
    ...
    const results = await Promise.all(
      files.map(async (f) => {
        const extractor =
          platform === 'codex'
            ? extractCodexPrompts
            : platform === 'claude-code' || platform === 'claude-desktop'
        ...
        try {
          const result = await extractor(f.path);
    ...
    for (const item of results) {
      if (item.error) {
        if (skippedFiles.length < 50) skippedFiles.push(item.file.path);
        if (warnings.length < 50) warnings.push(`failed to read ${item.file.path}: ${item.error.message}`);
        continue;
      }
      processedFiles++;
      if (item.result.prompts.length === 0) continue;
  ...
  return { platform, totalSessions, totalPrompts, groups, warnings, skippedFiles, processedFiles };
}
...
async function savePersistedAnalysis(platform, agentName, result) {
  await atomicWrite(analysisFilePath(platform, agentName), `${JSON.stringify(result, null, 2)}\n`);
}
