...
const CODEX_ARCHIVED_DIR = process.env.CODEX_ARCHIVED_DIR || path.join(HOME, '.codex', 'archived_sessions');
const CLAUDE_CODE_DIR = process.env.CLAUDE_CODE_DIR || path.join(HOME, '.claude', 'projects');
const CLAUDE_DESKTOP_DIR =
  process.env.CLAUDE_DESKTOP_DIR ||
...
function resolveDir(queryDir, defaultDir) {
  ...
  const resolved = path.resolve(queryDir);
  // User-supplied directories must stay within HOME to prevent path traversal.
  if (!isPathWithin(HOME, resolved)) return defaultDir;
  return resolved;
}

// Assert that a resolved absolute path stays within allowedRoot (default HOME).
// Performs a prefix check first, then — for paths that already exist — resolves
// symlinks via realpath() and re-verifies the real target is still in scope.
// Non-existent paths (e.g. a to-be-created archive dir) only get the prefix
// check because realpath requires the target to exist.
async function assertSafePath(resolvedPath, allowedRoot = HOME) {
  const root = path.resolve(allowedRoot);
  if (!isPathWithin(root, resolvedPath)) {
    throw new Error('Path escapes allowed scope');
  }
  try {
    const real = await fsp.realpath(resolvedPath);
    // Resolve the root through symlinks too (e.g. macOS /tmp -> /private/tmp)
    // so both sides are compared in realpath space. If the root itself does
    // not yet exist, fall back to its resolved path.
    let realRoot;
    try {
    ...
    if (!isPathWithin(realRoot, real)) {
      throw new Error('Path escapes allowed scope via symlink');
    }
  } catch (err) {
    if (err.code === 'ENOENT') return; // not yet created — prefix check suffices
    throw err;
  }
}
