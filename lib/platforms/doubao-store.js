...
async function refreshCache(inputDir, force = false) {
  ...
  const schemaIsCurrent = cacheSchemaIsCurrent();
  const now = Date.now();
  if (!force && schemaIsCurrent && now - lastRefreshAttempt < REFRESH_INTERVAL_MS) return CACHE_PATH;
  ...
  refreshPromise = (async () => {
    ...
    if (!force && schemaIsCurrent && signature && signature === lastSourceSignature) return CACHE_PATH;
...
async function ensureCache(inputDir) {
  if (fs.existsSync(CACHE_PATH)) {
    if (!cacheSchemaIsCurrent()) return refreshCache(inputDir, true);
...
function rowToMessage(row) {
  return {
    ...
    role: row.role,
    content: addEstimatedToolDurations(parseContent(row.content_json)),
    usage: null,
