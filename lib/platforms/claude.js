...
const { CLAUDE_CODE_DIR, resolveDir, assertSafePath } = require('../config');
const {
  withMetadataCache,
  withInFlightDedup,
  makeMessage,
...
async function listClaudeCodeProjects(baseDir) {
  const dir = resolveDir(baseDir, CLAUDE_CODE_DIR);
  let entries;
  try {
...
async function findClaudeCodeSessionFile(baseDir, sessionId) {
  ...
  for (const project of projects) {
    ...
    // Check subagents subdirectory — guard against symlink escape.
    const subDir = path.join(dirPath, 'subagents');
    let subFiles = [];
    try {
      const subLstat = await fsp.lstat(subDir);
      if (subLstat.isSymbolicLink()) {
        await assertSafePath(subDir, dirPath);
      }
      subFiles = await fsp.readdir(subDir, { withFileTypes: true });
    } catch {
      // no subagents dir or unsafe symlink — skip
    }
...
async function _parseClaudeCodeSessionFileRaw(filePath) {
  ...
  try {
    for await (const line of rl) {
      ...
      if (session && !session.timestamp && rec.timestamp && (t === 'user' || t === 'assistant')) {
