...
const { DOUBAO_DIR, ensureDirectory } = require('../config');
const { withMetadataCache, sortSessionsByTimestampDesc, topToolsOf, makeMessage, estimateToolDurationMs } = require('./shared');
const {
  indexedDbSourceFor,
  trajectoryRootFor,
  listCachedSessions,
...
async function listTrajectorySessions(baseDir) {
...
async function listDoubaoSessions(baseDir) {
  const inputDir = baseDir || DOUBAO_DIR;
  const trajectoryDir = trajectoryRootFor(inputDir);
  const [cached, trajectory] = await Promise.all([
    listCachedSessions(inputDir).catch(() => []),
    listTrajectorySessions(trajectoryDir),
  ]);
  const byId = new Map(trajectory.map((item) => [item.id, { ...item, dataSource: 'trajectory' }]));
  for (const item of cached) {
    const historical = byId.get(item.id);
    byId.set(item.id, {
      ...(historical || {}),
      ...item,
      messageCount: Math.max(item.messageCount || 0, historical?.messageCount || 0),
      userCount: Math.max(item.userCount || 0, historical?.userCount || 0),
      assistantCount: Math.max(item.assistantCount || 0, historical?.assistantCount || 0),
      toolCallCount: historical?.toolCallCount ?? item.toolCallCount ?? 0,
      toolResultCount: historical?.toolResultCount ?? item.toolResultCount ?? 0,
      topTools: historical?.topTools?.length ? historical.topTools : item.topTools || [],
      firstUserMessage: item.firstUserMessage || historical?.firstUserMessage || null,
      timestamp: item.timestamp || historical?.timestamp || null,
      lastActivity: item.lastActivity || historical?.lastActivity || item.timestamp || historical?.timestamp || null,
      trajectoryPath: historical?.file || null,
      sourcePath: item.file || null,
      dataSource: historical ? 'indexeddb+trajectory' : 'indexeddb',
    });
  }
  return sortSessionsByTimestampDesc([...byId.values()]);
}

async function findDoubaoSessionFile(baseDir, sessionId) {
  const dir = trajectoryRootFor(baseDir || DOUBAO_DIR);
  const sessionDir = path.join(dir, sessionId);
  try {
...
// Module-level memo cache for deterministic assistant-duration estimates.
const assistantDurationCache = new Map();
...
function estimateAssistantDurationMs(textLen) {
  const key = textLen || 0;
  const cached = assistantDurationCache.get(key);
  if (cached !== undefined) return cached;
  const result = !textLen ? 500 : Math.max(Math.round((textLen / 30) * 1000), 500);
  assistantDurationCache.set(key, result);
  return result;
}

// Module-level cache for parsed trajectory files, keyed by absolute path with
// mtime+size invalidation. Independent of the metadata cache in shared.js.
const doubaoSessionCache = new Map();

function clearDoubaoSessionCache() {
  doubaoSessionCache.clear();
}

async function parseDoubaoSessionFile(filePath) {
  // Cache lookup: skip re-parse if mtime and size are unchanged.
  try {
    const stat = await fsp.stat(filePath);
    const cached = doubaoSessionCache.get(filePath);
  ...
  const messages = rawMessages.map((msg, idx) => {
    ...
    return {
      ...
      model: null,
      provider: null,
      details: { ...(msg.details || {}), dataSource: 'trajectory', timestampEstimated: true },
    };
  });
  ...
  // Write cache entry with current mtime+size.
  try {
    const cacheStat = await fsp.stat(filePath);
    doubaoSessionCache.set(filePath, { mtimeMs: cacheStat.mtimeMs, size: cacheStat.size, result });
  } catch {
...
function messageMergeKey(message) {
  const text = (message.content || [])
    .map((item) => {
      if (item.type === 'text') return stripDoubaoNoise(item.text || '');
...
function mergeDoubaoMessages(trajectoryMessages, cachedMessages) {
  const cachedKeys = new Set(cachedMessages.map(messageMergeKey).filter(Boolean));
  const historical = trajectoryMessages.filter((message) => {
    const key = messageMergeKey(message);
    return !key || !cachedKeys.has(key);
  });
  return [...historical, ...cachedMessages].sort((a, b) => {
...
async function getDoubaoSession(baseDir, sessionId) {
  const inputDir = baseDir || DOUBAO_DIR;
  const trajectoryPath = await findDoubaoSessionFile(inputDir, sessionId);
  const [cached, trajectory] = await Promise.all([
    getCachedSession(inputDir, sessionId).catch(() => null),
    trajectoryPath ? parseDoubaoSessionFile(trajectoryPath).catch(() => null) : Promise.resolve(null),
  ]);
  if (!cached && !trajectory) return null;
  if (!cached) return trajectory;
  const messages = mergeDoubaoMessages(trajectory?.messages || [], cached.messages || []);
  const session = {
    ...(trajectory?.session || {}),
    ...cached.session,
    id: sessionId,
    trajectoryPath: trajectoryPath || null,
    sourcePath: cached.session.sourcePath || indexedDbSourceFor(inputDir),
    dataSource: trajectory ? 'indexeddb+trajectory' : 'indexeddb',
    historyAvailable: Boolean(trajectory),
...
module.exports = {
  ...
  estimateAssistantDurationMs,
  assistantDurationCache,
  parseDoubaoSessionFile,
  doubaoSessionCache,
  clearDoubaoSessionCache,
};
