...
const { CLAUDE_DESKTOP_DIR, resolveDir, assertSafePath } = require('../config');
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        // Resolve symlink and only descend if the real target stays within root.
        try {
          await assertSafePath(filePath, root);
          const real = await fsp.realpath(filePath);
          const stat = await fsp.stat(real);
          if (stat.isDirectory()) {
            if (!SKIP_DIRS.has(entry.name)) await walk(real);
          } else if (stat.isFile() && visitor(filePath, entry.name)) {
            out.push(filePath);
          }
        } catch {
          // broken symlink — skip
...
async function collectClaudeDesktopEntries(baseDir) {
  const dir = resolveDir(baseDir, CLAUDE_DESKTOP_DIR);
