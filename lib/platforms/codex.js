...
const { CODEX_DIR, CODEX_ARCHIVED_DIR, isPathWithin, HOME } = require('../config');
...
function normalizeCodexTokenUsage(rawUsage, contextWindow = null) {
  ...
  const usage = {
    input: number(rawUsage.input_tokens),
    output: number(rawUsage.output_tokens),
    cacheRead: number(rawUsage.cached_input_tokens),
    cacheWrite: number(rawUsage.cache_write_input_tokens),
    reasoning: number(rawUsage.reasoning_output_tokens),
...
async function collectCodexSessionEntries(baseDir, { includeChildren = true } = {}) {
  ...
  const [activeFiles, archivedFiles] = await Promise.all([listJsonlFiles(resolvedActive), listJsonlFiles(archivedDir)]);
