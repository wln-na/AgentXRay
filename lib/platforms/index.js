...
const { buildCodexContext, buildClaudeCodeContext, buildClaudeDesktopContext } = require('../context');
const PLATFORM_CAPABILITIES = require('./capabilities.json');
...
//   capabilities  boolean feature flags: search/context/backup/spawn/watch
...
function capabilities(platformId) {
  return Object.freeze({ ...CAPABILITY_DEFAULTS, ...(PLATFORM_CAPABILITIES[platformId] || {}) });
}
...
const PLATFORMS = {
    ...
    collectFiles: async (dir, opts = {}) => {
      ...
      for (const agent of agents) {
        ...
        for (const e of entries) {
          if (e.isFile() && isOpenClawSessionLogFile(e.name) && !isArchivedFile(e.name)) {
            files.push({
  ...
  codex: {
    ...
    contextBuilder: buildCodexContext,
    collectFiles: async (dir, opts = {}) => {
  ...
  'claude-code': {
    ...
    contextBuilder: buildClaudeCodeContext,
    collectFiles: async (dir, opts = {}) => {
      const files = [];
  ...
  'claude-desktop': {
    ...
    capabilities: capabilities('claude-desktop'),
    defaultDir: () => CLAUDE_DESKTOP_DIR,
    list: listClaudeDesktopSessions,
    find: findClaudeDesktopSessionFile,
    parse: parseClaudeDesktopSessionFile,
    getSession: getClaudeDesktopSession,
    contextBuilder: buildClaudeDesktopContext,
    collectFiles: async (dir) =>
      (await collectClaudeDesktopEntries(dir)).map((entry) => ({
        path: entry.filePath,
        file: entry.file,
        sessionId: entry.id,
    ...
    watchParse: (rec) => {
      const normalized = normalizeClaudeDesktopRecord(rec);
      return normalized ? { messages: [normalized] } : null;
    },
  },
  ...
  doubao: {
    id: 'doubao',
    label: 'Doubao',
    capabilities: capabilities('doubao'),
...
function platformSupports(platformId, capability) {
  const platform = PLATFORMS[platformId];
  return !!(platform && platform.capabilities && platform.capabilities[capability]);
}
