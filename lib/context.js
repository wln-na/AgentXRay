...
const { findClaudeDesktopSessionFile, getClaudeDesktopSession } = require('./platforms/claude-desktop');

// Supported platform → builder mapping lives in lib/platforms/index.js
// (PLATFORMS[platform].contextBuilder). New platforms register there.
// This module provides the shared entry point + common helpers + the
// platform-specific builder implementations themselves.
...
async function reconstructContext(platform, dir, sessionId, target = {}) {
  // Lazy require to avoid a circular dependency: platforms/index.js imports
  // the builder functions from this module at load time.
  const { PLATFORMS } = require('./platforms');
  const builder = PLATFORMS[platform]?.contextBuilder;
  if (!builder) {
...
/**
 * Keep only messages whose timestamp is strictly earlier than targetTs.
 * Messages without a timestamp are retained (they cannot be ordered by time).
 * Returns the array unchanged when targetTs is null/undefined.
 *
 * @param {Array} messages - normalized SessionMessage[]
 * @param {number|null} targetTs - epoch ms
 * @returns {Array}
 */
function filterMessagesBeforeTimestamp(messages, targetTs) {
  if (!targetTs) return messages;
  return messages.filter((m) => !m.timestamp || Date.parse(m.timestamp) < targetTs);
}
...
async function buildCodexContext(dir, sessionId, target) {
  ...
  const { targetIndex, targetMessage, priorMessages: rawPriorMessages } = sliceMessagesBeforeTarget(messages, target);

  // Apply strict timestamp filtering to priorMessages so that index-based
  // slicing and timestamp-based slicing agree when multiple records share
  // the same millisecond timestamp (Codex may write several response_item
  // records in the same ms). Messages without a timestamp are kept because
  // they cannot be ordered by time.
  const targetTs = targetMessage.timestamp ? Date.parse(targetMessage.timestamp) : null;
  const priorMessages = filterMessagesBeforeTimestamp(rawPriorMessages, targetTs);
  ...
  const messagesAfterCompaction = systemContext.compaction
    ? filterMessagesBeforeTimestamp(systemContext.messagesAfterCompaction, targetTs)
    : [];
...
module.exports = {
  ...
  filterMessagesBeforeTimestamp,
  buildMetadata,
  summarizeMessages,
  // Platform-specific builders (registered in lib/platforms/index.js)
  buildCodexContext,
  buildClaudeCodeContext,
  buildClaudeDesktopContext,
